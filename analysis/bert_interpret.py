...
class AdaptedRobertForSequenceRegression(nn.Module):
    ...
    def forward(self, input_ids, sents_per_school, attention_mask=None):
        ...
        outputs = self.bert(inputs, attention_mask=attention_mask) # [n_sent, dim]
        sent_embs = self.dropout(outputs[0].mean(dim=1)) # [n_sent, config.hidden_size]
        sent_embs = sent_embs.view(1, n_sent, sent_embs.size(-1))
        packed_sent_embs = torch.nn.utils.rnn.pack_padded_sequence(sent_embs, sents_per_school, enforce_sorted=False, batch_first=True)
        recurrent_output = self.gru(packed_sent_embs)[1].squeeze(0) # [1, recurrent_hidden_size]
...
def get_best_model(outcome):
	...
	if outcome in ['mn_avg_eb', 'mn_grd_eb']:
		...
		model = AdaptedRobertForSequenceRegression(config, hid_dim=hidden_dim, num_output=1, recurrent_num_layers=num_layers)
		for k in state_dict:
			curr_key = k
			if curr_key.startswith(('model.bert', 'model.fc1', 'model.output_layer')):
				curr_key = curr_key.split('model.')[1]
