...
group_by_school <- function(df_curr){
  df_g <- df_curr %>% 
    ...
    summarize(
      ...
      num_reviews = sum(!is.na( meta_comment_id )), 
      avg_review_len = mean(sapply(strsplit(as.character(review_text), " "), length), na.rm=T),
...
standardize_df_school <- function(df_curr_g){
  ...
  df_s <- df_curr_g %>%
    mutate(
      ...
      future_income = (future_income - mean(future_income, na.rm=T)) / sd(future_income, na.rm=T),
