...
def output_scores(
    ...
    all_data = {'url': [], 'progress_rating': [], 'test_score_rating': [
    ], 'equity_rating': [], 'overall_rating': [], 'school_name': []}
...
def get_overall_review_info(html):
    ...
    try:
        review_text = html.find(
            'div', attrs={'class': 'comment'}).get_text().encode('utf-8')
    except Exception as e:
        review_text = ''

    try:
        five_star_rating = len(html.findAll(
            'span', attrs={'class': 'icon-star filled-star'}))
...
def get_topical_review_info(topical_reviews):
    ...
    for curr_soup in topical_reviews:
        ...
        try:
            curr_rev['text'] = curr_soup.find(
                'div', attrs={'class': 'comment'}).get_text().encode('utf-8')
        except Exception as e:
            curr_rev['text'] = ''

        try:
            curr_rev['category'] = curr_soup.find(
                'span', attrs={'class': 'review-title-blue'}).get_text().lower().replace(' ', '_')
        except Exception as e:
            curr_rev['category'] = ''

        try:
            curr_rev['rating'] = convert_likert_to_number(curr_soup.find(
                'span', attrs={'class': 'review-grade-intro'}).get_text().lower().replace(':', ''))
        except Exception as e:
            curr_rev['rating'] = ''
...
def output_reviews(
    ...
    for i, f in enumerate(os.listdir(input_dir)):
        ...
        if not 'reviews' in curr or len(curr['reviews']) == 0:
            add_review_baseline_info(url, float('nan'), progress_rating, test_score_rating,
                                     equity_rating, overall_rating, float('nan'), float('nan'), float('nan'), all_data)
        ...
        for r in curr['reviews']:
            ...
            try:
                user_type = soup.find(
                    'div', attrs={'class': 'user-type'}).get_text()
            except Exception as e:
            ...
            # Date
            try:
                date = list(datefinder.find_dates(
                    soup.find('div', attrs={'class': 'type-and-date'}).get_text()))[0]
            except Exception as e:
                print('user date error: ', e)
                date = ''

            five_star_review = soup.find(
                'div', attrs={'class': 'five-star-review'})
            review_text, five_star_rating = get_overall_review_info(
                five_star_review)

            # Store all top level info
            add_review_baseline_info(url, meta_comment_id, progress_rating, test_score_rating,
                                     equity_rating, overall_rating, user_type, date, review_text, all_data)
            add_review_details('top_level', five_star_rating, all_data)

            # Next, topical reviews
            topical_reviews = soup.findAll(
                'div', attrs={'class': 'topical-review'})
            if len(topical_reviews) > 0:
                ...
                for t in parsed_topical:
                    ...
                    add_review_baseline_info(url, meta_comment_id, progress_rating, test_score_rating,
                                             equity_rating, overall_rating, user_type, date, t['text'], all_data)
...
def add_in_geo_info(
    ...
    df_school_tracts_covars = pd.merge(
        df_school_tracts, df_covars, how='left', on='tract_id')
    # df_final = pd.merge(df_school_tracts_covars, df_atlas, how='left', on='tract_id')
    df_final = df_school_tracts_covars

    print(len(df_base), len(df_tracts), len(df_covars),
          len(df_school_tracts), len(df_final))
...
def merge_gs_urls_with_seda(
    ...
    df_seda['schoolname'] = df_seda.apply(
        lambda x: update_seda_school_name(x.schoolname), axis=1)
    df_seda['schcity'] = df_seda.apply(
        lambda x: update_seda_school_city(x.schcity), axis=1)
    ...
    # Merge dataframes
    df = pd.merge(df_seda, df_gs, on=[
                  'schoolname', 'stateabb', 'schcity'], how='left')
