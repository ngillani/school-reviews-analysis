...
def compute_idf_scores_for_phrases(
	...
	for phrases in all_phrases:
		...
		df['num_phrase_occurrences'] = df.review_text.str.count(phrase_regex, re.I)
		...
		for i in range(1, N + 1):
			...
			curr_df = df[(df[outcome + '_percentile'] >= percentile_lowerbound) & (df[outcome + '_percentile'] < percentile_upperbound) ]
			data_to_output[str(percentile_upperbound*100) + '_idf'].append(np.log(curr_df['num_reviews'].sum() / curr_df['contains_phrase'].sum()))
...
def compare_attributions_to_tfidf_regression_weights(
	...
	for i, phrase in enumerate(attributions.keys()):
		if not phrase in regression_imp: continue
...
def combine_word_pieces(tokens, normalized_attributions, min_ngram=1, max_ngram=3, should_remove_stopwords=False):
	...
	for i, t in enumerate(tokens):
		...
		if should_skip_token(t): continue
		if i == 0 or '##' in t:
	...
	if min_ngram == -1 and max_ngram == -1:
		...
		for chunk in doc.noun_chunks:
			...
			if curr_ind < len(updated_token_list):
				phrase = ' '.join(updated_token_list[curr_ind:(curr_ind + len(np_words))])
				token_list_to_return.append(phrase)	
				phrase_to_sent[phrase] = sent_str
				attributions_to_return.append(np.sum(updated_attributions[curr_ind:(curr_ind + len(np_words))]))
				
	if max_ngram > 1:
		token_list_to_return, attributions_to_return = combine_ngrams(updated_token_list, updated_attributions, min_ngram=min_ngram, max_ngram=max_ngram)
...
def analyze_attributions(
	...
	for num, f in enumerate(os.listdir(input_dir)):
		...
		for sent in curr:
			updated_tokens, updated_attr, curr_phrase_to_sent = combine_word_pieces(curr[sent]['tokens'], curr[sent]['attributions'], min_ngram=min_ngram, max_ngram=max_ngram, should_remove_stopwords=should_remove_stopwords)
			...
			for i in range(0, len(updated_tokens)):
				...
				token_to_sent[updated_tokens[i]].append((num, curr_phrase_to_sent[updated_tokens[i]], updated_attr[i]))

		for t in attr_per_token:
			if t in to_exclude: continue
			total_attr_per_token[t].append((num, attr_per_token[t]))
			school_per_token[t].add(f)

	tokens_to_include = list(filter(lambda x: len(school_per_token[x]) >= MIN_SCHOOL_OCCUR, school_per_token.keys()))
	...
	sorted_imp = sorted(total_word_imp.items(), key=lambda x:np.abs(x[1]), reverse=True)
	...
	write_dict(output_file.format(model_key, min_ngram, max_ngram), dict(sorted_imp))
	write_dict(output_file2.format(model_key, min_ngram, max_ngram), {t: token_to_sent[t] for t in tokens_to_include})
...
def output_ngram_scatter_plot_vals(
	...
	attributions1 = read_dict(attributions_file.format(model_1_key, min_ngram, max_ngram))
	attributions2 = read_dict(attributions_file.format(model_2_key, min_ngram, max_ngram))
	
	common_ngrams = list(set(attributions1.keys()).intersection(set(attributions2.keys())))
	output_data = {
		'ngram': [],
		'model_1_attr': [],
		'model_2_attr': []
...
def load_and_encode_ngrams(
	...
	sorted_attributions = list(filter(lambda x:np.abs(x[1]) >= sd_threshold, attributions.items()))
...
def cluster_attribution_ngrams(
	...
	phrase_occurrences = read_dict(phrase_occurrences_file.format(model_key, min_ngram, max_ngram))
	...
	for i in range(0, len(ngrams)):
		ngram_cluster_assignments[cluster_labels[i]].append((ngrams[i], attr_vals[i]))

	print ('Computing median attribution value per cluster')
	cluster_stats = {'weighted_mean': {}, 'weighted_mean_abs': {}, 'weighted_sd': {}, 'phrases': {}, 'attr_vals': {}}
	aux_cluster_id = -1
	...
	for c in ngram_cluster_assignments:
		...
		cluster_stats['phrases'][c] = ','.join(ngram_cluster_assignments[c][:,0].tolist())
		cluster_stats['attr_vals'][c] = ','.join([str(v) for v in ngram_cluster_assignments[c][:,1].tolist()])

	sorted_clusters = sorted(cluster_stats['weighted_mean'].items(), key=lambda x:np.abs(x[1]))
	data_to_output = {
		'cluster_id': [],
		'ngrams': [],
		'ngram_attributions': [],
	...
	for c in sorted_clusters:
		...
		data_to_output['ngram_attributions'].append(cluster_stats['attr_vals'][c[0]])
		data_to_output['weighted_mean_attribution'].append(c[1])
		data_to_output['weighted_mean_attribution_abs'].append(np.abs(c[1]))
		data_to_output['weighted_sd_attribution'].append(cluster_stats['weighted_sd'][c[0]])

	df = pd.DataFrame(data=data_to_output)

	df['mean_attr_percentile'] = df['weighted_mean_attribution'].rank(pct=True)
	df['trimmed_ngrams'] = df.apply(lambda x: ','.join(x.ngrams.split(',')[:3]), axis=1)
	df.sort_values(by='weighted_mean_attribution_abs', ascending=False, inplace=True)
...
def compute_bias_score(neutral_words, neutral_word_weights, mean_baseline_vec, mean_target_vec):
	...
	for i in range(0, len(neutral_words)):
		...
		try:
			curr_bias = 1 - cosine(neutral_words[i], mean_baseline_vec)
			biases.append(curr_bias)
			importances.append(np.abs(neutral_word_weights[i]))
...
def compute_relative_bias_score(neutral_words, neutral_word_weights, mean_baseline_vec, mean_target_vec):
	...
	for i in range(0, len(neutral_words)):
		...
		try:
			curr_bias = np.linalg.norm(neutral_words[i] - mean_baseline_vec) - np.linalg.norm(neutral_words[i] - mean_target_vec)
			biases.append(curr_bias)
			importances.append(np.abs(neutral_word_weights[i]))
			# importances.append(1)
		except Exception as e:
...
def compute_linguistic_bias(
		...
		max_ngram=-1,
		sd_threshold=0,
		bias_score_function=compute_relative_bias_score,
	):
	...
	print ('Loading and encoding ngram data model 2 ...')
	ngrams2, attr_vals2, encoded_ngrams2 = load_and_encode_ngrams(encoder, model_key=model_2_key, min_ngram=min_ngram, max_ngram=max_ngram, sd_threshold=sd_threshold)
	print (len(ngrams2))

	baseline_words = ['pta', 'involved parents', 'parental involvement', 'pto']
	target_words = ['amazing teachers', 'excellent teachers', 'great teachers']

	# baseline_words = ['gifted program', 'gifted and talented program']
	# target_words = ['math', 'reading']

	# baseline_words = ['art', 'music']
	# target_words = ['math', 'reading']

	# baseline_words = ['my child is happy', 'my child is satisfied', 'my child is fulfilled', 'my child is joyful', 'my child is having fun']
	# target_words = ['my child is learning and growing', 'child is developing intellectually']

	print ('Encoding baseline and target words ...')
	mean_baseline_vec = np.mean(encoder.encode(baseline_words), axis=0)
	mean_target_vec = np.mean(encoder.encode(target_words), axis=0)

	biases_per_group = {}
	biases_per_group['1'] = bias_score_function(encoded_ngrams1, attr_vals1, mean_baseline_vec, mean_target_vec)
	biases_per_group['2'] = bias_score_function(encoded_ngrams2, attr_vals2, mean_baseline_vec, mean_target_vec)

	from scipy.stats import mannwhitneyu, ks_2samp
	groups = sorted(list(biases_per_group.keys()))

	print ('Median group 1 is {}, group 2 is {}'.format(np.median(biases_per_group['1']), np.median(biases_per_group['2'])))
	for i in range(0, len(groups)):
		for j in range(i + 1, len(groups)):
			print(groups[i], groups[j], mannwhitneyu(biases_per_group[groups[i]], biases_per_group[groups[j]], alternative='two-sided'))
			print(groups[i], groups[j], ks_2samp(biases_per_group[groups[i]], biases_per_group[groups[j]]))
